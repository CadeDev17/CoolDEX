...
export default function Home() {
  ...
  // `reservedCD` keeps track of the Cool DAO tokens Reserve balance in the Exchange contract
  ...
  // removeCD is the amount of `Cool DAO` tokens that would be sent back to the user based on a certain number of `LP` tokens
  ...
  const [tokenToBeReceivedAfterSwap, settokenToBeReceivedAfterSwap] =
    useState(zero);
  // Keeps track of whether  `Eth` or `Cool DAO` token is selected. If `Eth` is selected it means that the user
  // wants to swap some `Eth` for some `Cool DAO` tokens and vice versa if `Eth` is not selected
  ...
  const getAmounts = async () => {
    try {
      ...
      // get the amount of `Cool DAO` tokens held by the user
      const _cdBalance = await getCDTokensBalance(provider, address);
      // get the amount of `Cool DAO` LP tokens held by the user
  ...
  /**
   * swapTokens: Swaps  `swapAmountWei` of Eth/Cool DAO tokens with `tokenToBeReceivedAfterSwap` amount of Eth/Cool DAO tokens.
   */
  ...
  /**
   * _getAmountOfTokensReceivedFromSwap:  Returns the number of Eth/Cool DAO tokens that can be received
   * when the user swaps `_swapAmountWEI` amount of Eth/Cool DAO tokens.
   */
   ...
   * then we calculate the Cool DAO tokens he can add, given the Eth he wants to add by keeping the ratios
   * constant
   */
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      const _ethBalance = await getEtherBalance(provider, null, true);
      // get the Cool DAO token reserves from the contract
      ...
      return (
        <div>
          <div className={styles.description}>
            ...
            {utils.formatEther(cdBalance)} Cool DAO
            <br />
            {utils.formatEther(ethBalance)} Ether
            <br />
            {utils.formatEther(lpBalance)} Cool DAO LP tokens
          </div>
          <div>
            ...
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                ...
                <input
                  type="number"
                  placeholder="Amount of Cool DAO tokens"
                  onChange={(e) =>
                    setAddCDTokens(
              ...
              <div>
                ...
                <div className={styles.inputDiv}>
                  {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                  {`You will need ${utils.formatEther(addCDTokens)} Cool DAO
                  Tokens`}
                </div>
                <button className={styles.button1} onClick={_addLiquidity}>
                  Add
      ...
      return (
        <div>
          ...
          >
            ...
            <option value="coolDAOToken">Cool DAO Token</option>
          </select>
          <br />
          <div className={styles.inputDiv}>
            {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
